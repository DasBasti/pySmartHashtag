...
class SmartAuthentication(httpx.Auth):
    ...
    def clear_credentials(self) -> None:
        ...
        Call this method when you no longer need to perform fresh logins
        and want to minimize the exposure of credentials in memory.
        Note: After calling this, re-authentication will fail if the
        refresh token expires or becomes invalid.

        This clears:
        - Password (used for initial login)
        - Username (used for initial login)
        """
        self.password = None
        self.username = None
        _LOGGER.debug("Credentials cleared from memory")
    ...
    async def _refresh_access_token(self):
        """Refresh the EU access token using the token endpoint.
        
        EU refresh flow:
        - POST to /login-app/api/v1/token with {accessToken, refreshToken}
        - Response: {access_token, refresh_token, id_token}
        - Then exchange new access_token for vehicle API session
        """
        if not self.refresh_token or not self.access_token:
            _LOGGER.debug("No refresh token available, will perform full login")
            return {}
            
        try:
            ssl_ctx = await self.get_ssl_context()
            async with SmartLoginClient(ssl_context=ssl_ctx) as client:
                _LOGGER.debug("EU: Refreshing access token via token endpoint")
                ...
                r_refresh = await client.post(
                    ...
                    headers={
                        ...
                        "User-Agent": "Hello smart/2.0.5 (iPhone; iOS 17.1; Scale/3.00)",
                    },
                )
                
                refresh_result = r_refresh.json()
                _LOGGER.debug("EU refresh response: %s", sanitize_log_data(refresh_result))
                
                # Response has snake_case keys: access_token, refresh_token, id_token
                access_token = refresh_result.get("access_token")
                refresh_token = refresh_result.get("refresh_token")
                
                if not access_token or not refresh_token:
                    _LOGGER.warning("EU refresh failed: missing tokens in response")
                    return {}
                
                _LOGGER.info("EU: Token refresh successful")
                
                # Now exchange the new access token for vehicle API session
                # (same as final step of login)
                ...
                _LOGGER.debug("EU API session result: %s", sanitize_log_data(api_result))
                ...
                # Use existing expiry or default to 24 hours
            ...
            _LOGGER.debug("EU token refresh failed: %s. Will perform full login.", e)
            return {}

    async def _login(self):
        """Login to Smart web services."""
        # Check if credentials are available (may have been cleared)
        if not self.password or not self.username:
            raise SmartAPIError(
                "Cannot login: credentials have been cleared. "
                "Create a new authentication instance with valid credentials."
            )
...
class SmartAuthenticationINTL(SmartAuthentication):
    ...
    def _generate_intl_headers(self, auth_token: str = "") -> dict[str, str]:
        ...
        headers = {
            ...
            "X-Ca-Signature-Headers": "Accept-Language,User-Agent,X-Ca-Nonce,X-Ca-Timestamp,Xs-App-Ver,Xs-Auth-Token,Xs-Client-Id,Xs-Di,Xs-Os,Xs-Session-Id,Xs-Ui",
            "Xs-Os": "iOS",
            "Xs-App-Ver": "1.0.7",
    ...
    def _generate_vehicle_api_headers(self, body: str = None) -> dict[str, str]:
        ...
        if body is not None:
            headers["x-signature"] = utils._create_sign(
                ...
                url=API_SESION_URL,
                body=body,
                use_intl=True,  # Use INTL signing secret
            )
    ...
    async def _refresh_access_token(self):
        """Refresh the INTL access token using the IAM refresh endpoint.
        
        INTL refresh flow:
        - POST to /iam/service/api/v1/refresh/ with {refreshToken}
        - Response: {code: "200", result: {accessToken, refreshToken, idToken, expiresIn, userId}}
        - Then exchange new accessToken for authCode via OAuth
        - Finally exchange authCode for vehicle API session
        """
        if not self.intl_refresh_token:
            _LOGGER.debug("INTL: No refresh token available, will perform full login")
            return {}
            
        try:
            ssl_ctx = await self.get_ssl_context()
            async with SmartLoginClient(ssl_context=ssl_ctx) as client:
                _LOGGER.debug("INTL: Refreshing access token via IAM refresh endpoint")
                
                refresh_data = json.dumps({
                    "refreshToken": self.intl_refresh_token,
                })
                
                r_refresh = await client.post(
                    INTL_REFRESH_URL,
                    content=refresh_data,
                    headers=self._generate_intl_headers(self.intl_id_token or ""),
                )
                
                refresh_result = r_refresh.json()
                _LOGGER.debug("INTL refresh response code: %s", refresh_result.get("code"))
                
                if refresh_result.get("code") != "200":
                    error_msg = refresh_result.get("message", "Unknown error")
                    _LOGGER.warning("INTL refresh failed: %s", error_msg)
                    return {}
                ...
                _LOGGER.info("INTL: Token refresh successful for user %s", self.intl_user_id)
                
                # Step 2: Exchange new accessToken for authCode via OAuth
                _LOGGER.debug("INTL: Exchanging refreshed accessToken for authCode")
                ...
                if oauth_result.get("code") != "200":
                    error_msg = oauth_result.get("message", "Unknown error")
                    _LOGGER.warning("INTL OAuth after refresh failed: %s", error_msg)
                    return {}
                ...
                _LOGGER.debug("INTL: Got authCode from refresh flow")
                
                # Step 3: Exchange authCode for vehicle API session
                _LOGGER.debug("INTL: Exchanging authCode for vehicle API session")
                ...
                if session_result.get("code") != 1000:
                    error_msg = session_result.get("message", "Unknown error")
                    _LOGGER.warning("INTL session after refresh failed: %s", error_msg)
                    return {}
                ...
                _LOGGER.info("INTL: Refresh flow completed successfully")
            ...
            _LOGGER.debug("INTL token refresh failed: %s. Will perform full login.", e)
            return {}
