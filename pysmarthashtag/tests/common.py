...
from pysmarthashtag.const import API_BASE_URL, API_CARS_URL, API_SELECT_CAR_URL, API_SESION_URL, AUTH_URL, CONTEXT_URL, LOGIN_URL
...
class SmartMockRouter(respx.MockRouter):
    ...
    def add_login_routes(self) -> None:
        ...
        # Login context
        self.get(CONTEXT_URL).respond(
            302,
            headers={"location":load_response(RESPONSE_DIR / "auth_context.url")}
        )
        ...
        self.get(AUTH_URL+"?context=eu1_tk1.WTygkw2cU6QLANxlBVba0lP4ndHARZgp14RhsVBRiVE.1705656141&login_token=TestToken").respond(
            302,
            headers={"location":"https://auth.smart.com"+load_response(RESPONSE_DIR / "auth_result.url")}
        )
        ...
        self.post(API_BASE_URL+API_SELECT_CAR_URL).respond(
            200,
            json={}
        )
        self.get(API_BASE_URL+"/remote-control/vehicle/status/TestVIN0000000001?latest=True&target=basic%2Cmore&userId=112233").respond(
            200,
