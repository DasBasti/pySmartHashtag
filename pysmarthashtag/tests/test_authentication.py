...
async def test_login_global():
    ...
    with respx.mock:
        ...
        login_route = respx.post(
            GLOBAL_API_BASE_URL + "/iam/service/api/v1/login"
        ).mock(return_value=Response(200, json=load_response(RESPONSE_DIR / "global_login_result.json")))
        
        # Mock API session endpoint
        respx.post(
            GLOBAL_API_BASE_URL + "/iam/service/api/v1/session?identity_type=smart"
        ).mock(return_value=Response(200, json=load_response(RESPONSE_DIR / "api_access.json")))
...
async def test_refresh_access_token_global():
    ...
    with respx.mock:
        ...
        refresh_route = respx.post(
            GLOBAL_API_BASE_URL + "/iam/service/api/v1/refresh/"
        ).mock(return_value=Response(200, json=load_response(RESPONSE_DIR / "global_refresh_result.json")))
